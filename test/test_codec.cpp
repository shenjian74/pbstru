...
int main(int argc, char* argv[])
{
...

    {
        ...
        assert(true == rewrite_varint(buf, offset, 1));
        offset = 0;
	decode_varint(buf, &value, &offset);
	assert(1 == offset);
	assert(1 == value);
        assert(true == rewrite_varint(buf, offset, 127));
        offset = 0;
	decode_varint(buf, &value, &offset);
	assert(1 == offset);
	assert(127 == value);
        assert(false == rewrite_varint(buf, offset, 128));
	...
	assert(2 == offset);
	assert(127 == value);
        assert(true == rewrite_varint(buf, offset, 128));
        assert(false == rewrite_varint(buf, offset, 65535));
	...
	assert(127 == value);
        assert(true == rewrite_varint(buf, offset, 128));
        assert(true == rewrite_varint(buf, offset, 65535));
	...
	assert(5 == offset);
	assert(65535 == value);
        assert(true == rewrite_varint(buf, offset, 0xFFFFFFFE));
        offset = 0;
	decode_varint(buf, &value, &offset);
	assert(5 == offset);
...

    {
        ...
        buf_len = encode_message_Tuple(&(var_Tuple), buf, sizeof(buf));
...

    {
        ...
        buf_len = encode_message_AddRequest(&var_AddRequest, buf, sizeof(buf));
        print_buffer(buf, buf_len);
...
/*
    {
        ...
        for(int i=0; i<3; ++i)
        {
            ...
            size_t size1 = encode_message_ut_test_message(msg, NULL, sizeof(buf));
...

    {
        ...
        for(int i=0; i<3; ++i)
        {
            ...
            size_t size1 = encode_message_ut_test_sub_message(msg, NULL, sizeof(buf));

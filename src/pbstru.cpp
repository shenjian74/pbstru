...
int gen_comm(const string &target_dir)
{
    ...
    fprintf(fp, "#endif\n");
    fprintf(fp, "\n");
    fprintf(fp, "#define encode_tag_byte(buf, tag, wire_type, offset) encode_tag_byte_%s((buf), (tag), (wire_type), (offset))\n", _BUILD_TIME_);
    fprintf(fp, "void encode_tag_byte_%s(BYTE *buf, const BYTE tag, const BYTE wire_type, size_t *offset);\n", _BUILD_TIME_);
    fprintf(fp, "\n");

    fprintf(fp, "#define parse_tag_byte(buf, buflen, field_num, wire_type, offset) parse_tag_byte_%s((buf), (buflen), (field_num), (wire_type), (offset))\n", _BUILD_TIME_);
    fprintf(fp, "BOOL parse_tag_byte_%s(const BYTE* buf, const size_t buflen, WORD *field_num, BYTE *wire_type, size_t *offset);\n", _BUILD_TIME_);
    fprintf(fp, "\n");
    fprintf(fp, "#ifdef __cplusplus\n");
    fprintf(fp, "}\n");
    fprintf(fp, "#endif\n");
...
static int gen_header(const Descriptor *desc, string &target_dir, map<string, string> &map_array_size)
{
    int retcode = 0;
...
static int gen_source(const Descriptor *desc, string &target_dir, const map<string,string> &map_array_size)
{
    int retcode = 0;
    string name_lower = desc->name();
    tolower(name_lower);
    ...
    for(int i=0; i<desc->field_count(); ++i)
    {
        ...
        if(field->is_repeated())
        {
            if(field->is_packed())
            {
                ...
                case FieldDescriptor::TYPE_FIXED32:
                case FieldDescriptor::TYPE_FLOAT:
                    fprintf(fp, "        encode_tag_byte_%s(buf, %d, WIRE_TYPE_FIX32, &offset);\n", _BUILD_TIME_, field->number());
                    break;
                case FieldDescriptor::TYPE_FIXED64:
                    fprintf(fp, "        encode_tag_byte_%s(buf, %d, WIRE_TYPE_FIX64, &offset);\n", _BUILD_TIME_, field->number());
                    break;
                case FieldDescriptor::TYPE_BOOL:
                case FieldDescriptor::TYPE_UINT32:
                case FieldDescriptor::TYPE_UINT64:
                case FieldDescriptor::TYPE_ENUM:
                case FieldDescriptor::TYPE_INT32:
                    fprintf(fp, "        encode_tag_byte_%s(buf, %d, WIRE_TYPE_VARINT, &offset);\n", _BUILD_TIME_, field->number());
                    break;
                default:
                    break;
                }
        ...
        case FieldDescriptor::TYPE_FIXED32:
            if(!field->is_packed())
            {
                fprintf(fp, "%sencode_tag_byte_%s(buf, %d, WIRE_TYPE_FIX32, &offset);\n", prefix_spaces.c_str(), _BUILD_TIME_, field->number());
            }
        ...
        case FieldDescriptor::TYPE_INT32:
            if(!field->is_packed())
            {
                fprintf(fp, "%sencode_tag_byte_%s(buf, %d, WIRE_TYPE_VARINT, &offset);\n", prefix_spaces.c_str(), _BUILD_TIME_, field->number());
            }
        ...
        case FieldDescriptor::TYPE_STRING:
            if(field->is_repeated())
            {
                fprintf(fp, "%sencode_tag_byte_%s(buf, %d, WIRE_TYPE_LENGTH_DELIMITED, &offset);\n", prefix_spaces.c_str(), _BUILD_TIME_, field->number());
...
// Decode function
    ...
    fprintf(fp, "        if(FALSE == parse_tag_byte_%s(buf+offset, buf_len-offset, &field_num, &wire_type, &offset)) {\n", _BUILD_TIME_);
    fprintf(fp, "            return FALSE;\n");
    fprintf(fp, "        }\n");

...
void gen_comm(void) {
	...
	fprintf(fp, "	size_t length;\n");
	fprintf(fp, "} ps_bytes;\n");
	fprintf(fp, "\n#define parse_tag_byte(buf, field_num, wire_type, offset){ \\\n");
	...
	fprintf(fp, "} \n");
	fprintf(fp, "\n");
	fprintf(fp, "#define encode_tag_byte(buf, tag, wire_type, offset) { \\\n");
	fprintf(fp, "	if (tag < 16) { \\\n");
	fprintf(fp, "		if (NULL != buf) { \\\n");
	fprintf(fp, "			buf[(*(offset))] = (((BYTE)tag) << 3) | ((BYTE)wire_type); \\\n");
	fprintf(fp, "		} \\\n");
	fprintf(fp, "		(*(offset)) += 1; \\\n");
	fprintf(fp, "	} \\\n");
	fprintf(fp, "	else { \\\n");
	...
	fprintf(fp, "			buf[(*(offset)) + 1] = ((BYTE)(tag >> 4) & 0x78) | ((BYTE)wire_type); \\\n");
	fprintf(fp, "		} \\\n");
	fprintf(fp, "		(*(offset)) += 2; \\\n");
	fprintf(fp, "	} \\\n");
	fprintf(fp, "} \n");
	...
	fprintf(fp, "			if (NULL != buf) { \\\n");
	fprintf(fp, "				buf[(*(offset)) + i] = ((BYTE)remain_len) | 0x80; \\\n");
	fprintf(fp, "			} \\\n");
	fprintf(fp, "			remain_len = remain_len >> 7; \\\n");
	fprintf(fp, "		} \\\n");
	fprintf(fp, "		else { \\\n");
	fprintf(fp, "			if (NULL != buf) { \\\n");
	fprintf(fp, "				buf[(*(offset)) + i] = (BYTE)remain_len; \\\n");
	fprintf(fp, "			} \\\n");
	fprintf(fp, "			break; \\\n");
	fprintf(fp, "		} \\\n");
	fprintf(fp, "	} \\\n");
	fprintf(fp, "	*(offset) += 1 + i; \\\n");
	fprintf(fp, "}\n");
	fprintf(fp, "\n");
	...
	fprintf(fp, "    (*(value)) = 0; \\\n");
	fprintf(fp, "    for(i=0;;++i){ \\\n");
	fprintf(fp, "	     (*(value)) += ((buf)[i] & 0x7F) << (7*i); \\\n");
	fprintf(fp, "        if(0 == ((buf)[i] & 0x80)){ \\\n");
	fprintf(fp, "            break; \\\n");
	fprintf(fp, "        } \\\n");
	fprintf(fp, "    } \\\n");
	fprintf(fp, "    (*(offset)) += 1 + i; \\\n");
	fprintf(fp, "}\n");
	fprintf(fp, "\n");
	fprintf(fp, "#define deal_unknown_field(wire_type, buf, offset) { \\\n");
